...
namespace VEngine
{
    ...
    public sealed class RawFile : Loadable
    {
        ...
        protected override void OnLoad()
        {
            ...
            {
                var asset = manifest.GetAsset(name);
                if (asset == null)
                {
                    continue;
                }
                info = manifest.GetBundle(asset.bundle);
                if (info == null)
                {
                    break;
                }
                pathOrURL = manifest.includeInBuild
                    ? Versions.GetPlayerDataURL(info.name)
                    : Versions.GetDownloadURL(info.name);
