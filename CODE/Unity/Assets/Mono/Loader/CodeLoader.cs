    ...
    public class CodeLoader : IDisposable
    {
        ...
        public async ETTask Start()
        {
            switch (this.CodeMode)
            {
                case CodeMode.Mono:
                    {
                        byte[] assBytes = ((await Asset.LoadAsync("Assets/Bundles/Code/Code.dll.bytes", typeof(TextAsset)).ETAsync()).asset as TextAsset).bytes;
                        byte[] pdbBytes = ((await Asset.LoadAsync("Assets/Bundles/Code/Code.pdb.bytes", typeof(TextAsset)).ETAsync()).asset as TextAsset).bytes;
