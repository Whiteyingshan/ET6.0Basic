...
const SystemComponentType: string[] = ["GGraph", "GGroup", "GImage", "GList", "GLoader", "GLoader3D", "GMovieClip", "GRichTextField", "GTextField", "GTextInput"]

function genCode(handler: FairyEditor.PublishHandler) {
    const settings = (handler.project.GetSettings("Publish") as FairyEditor.GlobalPublishSettings).codeGeneration;
    //转换为中文拼音,删除特殊字符
    ...
    for (let i = 0; i < classes.Count; i++) {
        ...
        const crossPackageFlagsArray = new Array<boolean>();
        const customComponentFlagsArray = new Array<boolean>();
